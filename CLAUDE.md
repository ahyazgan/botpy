...
- **Manuel:** `place_trade`. **Otomatik:** `maybe_auto_trade` → yan etkisiz `auto_decision` (güç ≥ `auto_min_impact` + `confirmed` + cooldown + limitler + spot-short-yok + chase/kaynak filtreleri + boyut) ile karar verir, uygunsa `place_trade`. `auto_decision` global `auto_trade` anahtarından bağımsızdır → `/auto-preview` (dry-run) kalibrasyon önizlemesi sunar. **Conviction sizing** (`size_by_impact`): açıksa oto-işlem boyutu güce göre ölçeklenir (`_size_multiplier`: güç 8→1.0x, 10→1.5x, 7→0.75x, [0.5x,1.5x]).
